...
def rk45_step(dxdt, x, t, h, eps=1e-5):
    """
...
for i in range(s):
    dxdt = lambda x,t : f(x, params_arr[i], t)
    x_output, t_output = rk45_solve(dxdt, x0_arr[i,:], t0, t_end)
    x_rk45.append(x_output)
    t_rk45.append(t_output)

x_rk45 = np.array(x_rk45)
...
# plt.plot(t_rk45[0], x_rk45[0][:,0], label = "rk45", linestyle="-", alpha = .3)
# plt.legend()
