...
if __name__ == "__main__":
    ...
    run_odeint_comparison = True
    show_plots = False
    blockSize = 128
    s = 128*64 # number of odes to solve
    h = .01 # step size - todo: make different per ode
    t0 = 0 # start time
    t_end = 25 # end time
    ###########################################################

    n_timesteps = (t_end-t0)//h + 1
    odeint_time = None
    cuda_time = None
    ...
    if run_odeint_comparison or show_plots:
        timestamp = time.time()
        t_odeint = np.linspace(t0,t_end,int(n_timesteps))
    ...
    if write_to_csv_flag or show_plots:
        ...
        with open ("rk45_fixed_output.csv") as f:
            data = f.readlines()
    ...
    # # for i in range(s):
    # #     dxdt = lambda x,t : f(x, p_arr[i])
    # #     x_output, t_output = rk45solver.solve(dxdt, x0_arr[i,:], t0, t_end, h)
    # #     x_rk45.append(x_output)
    # #     t_rk45.append(t_output)
    # #
    # # x_rk45 = [np.array(arr) for arr in x_rk45]
    # # print(f"rk45 py total time: {round(1000*(time.time()-timestamp), 2)} ms")
